...
class EmotionPredictor:
    ...
    def predict_classes(self, reviews, ratings, authors, hotels):
        indices = self._review_to_indices(reviews)
        predictions = self.model.predict(indices, verbose=False)

        df = pd.DataFrame({'Author': authors, 'Rating': ratings, 'Review': reviews, 'Hotel': hotels})
        if self.setting == 'mc':
            ...
            for emotion, values in zip(self.class_values, predictions.T):
                df[emotion] = values
        #df.to_json ('results/predictions.json', orient='records')
        return df

    def predict_probabilities(self, reviews, ratings, authors, hotels):
        indices = self._review_to_indices(reviews)
        predictions = self.model.predict(indices, verbose=False)
        ...
        for emotion, values in zip(self.class_values, predictions.T):
            df[emotion] = values
        #df.to_json('results/probabilities.json', orient='records')
        return df.to_json(orient='records')
        #return df

    def embed(self, reviews, ratings, authors, hotels):
        indices = self._review_to_indices(reviews)
        ...
        #df.to_json('results/embed.json', orient='records')
        return df
